...
class SettingsSlideFragment : PoolFragment() {
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        openGroupsExpandedSettingsSwitch.isChecked = on<SettingsHandler>().get(CLOSER_SETTINGS_OPEN_GROUP_EXPANDED)
        openGroupsExpandedSettingsSwitch.setOnCheckedChangeListener { v, checked -> on<SettingsHandler>().set(CLOSER_SETTINGS_OPEN_GROUP_EXPANDED, checked) }
        ...
        view.findViewById<View>(R.id.returnToMapButton).setOnClickListener { v -> on<MapActivityHandler>().goToScreen(EXTRA_SCREEN_MAP) }
